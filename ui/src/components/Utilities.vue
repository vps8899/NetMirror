  ...
  <Teleport to="body">
    ...
    >
      <div v-if="toolComponentShow" class="fixed inset-0 z-50 overflow-hidden">
        ...
        >
          <div v-if="toolComponentShow" class="absolute right-0 top-0 h-full w-full max-w-3xl bg-gray-100 shadow-2xl flex flex-col">
            <!-- Header -->
            <div class="flex items-center justify-between p-4 border-b border-gray-200 flex-shrink-0">
              <div class="flex items-center space-x-4">
                ...
                <div>
                  <h2 class="text-xl font-semibold text-gray-900">{{ currentTool?.label }}</h2>
                  <p class="text-sm text-gray-600">{{ currentTool?.description }}</p>
                </div>
              </div>
              <button
                @click="closeTool"
                class="flex items-center justify-center w-10 h-10 rounded-full bg-gray-200 hover:bg-gray-300 transition-colors duration-200"
              >
